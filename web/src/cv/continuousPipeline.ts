// Ties together the three continuous-mode CV pieces (videoPoseTrack,
...
import { renderSkeletalPuppetFrameFromPoses } from "./limbWarp";
 ...
 * `refImage`/`refPose`/`refSize` describe the single anatomy reference
 * image already aligned to one frame (the same image/pose the single-freeze
 * flow uses); `targetSize` is the output resolution (normally the source
 * video's own). A frame where pose or mask detection dropped out carries
 * the nearest earlier frame's result forward rather than aborting — a
 * momentary CV miss shouldn't sink the whole range.
 */
export async function buildContinuousFrames(
  ...
  for (let i = 0; i < poseFrames.length; i++) {
    ...
    const puppetCanvas = renderSkeletalPuppetFrameFromPoses(refImage, refPose, pose, refSize, targetSize);
    frames.push({
